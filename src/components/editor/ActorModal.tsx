...
export function ActorModal({ isOpen, onClose, onSave, position, initialData, currentActorCount }: ActorModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    actorInitials: sanitizeInitials(initialData?.actorInitials || ''),
  ...
  const validateForm = (): boolean => {
    ...
    } else if (!/^[A-Z0-9]{2}$/.test(initials)) {
      newErrors.actorInitials = 'Use exatamente 2 caracteres alfanuméricos'
    }
  ...
  const handleInputChange = (field: string, value: string) => {
    setFormData(prev => {
      ...
      if (field === 'actorInitials') {
        newData.actorInitials = sanitizeInitials(value)
        return newData
      }
      ...
      if (field === 'actorName' && value.trim()) {
        ...
        if (!currentInitials || currentInitials === autoGeneratedInitials) {
          newData.actorInitials = sanitizeInitials(generateInitials(value.trim()))
        }
      }
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => handleInputChange('actorInitials', e.target.value)}
